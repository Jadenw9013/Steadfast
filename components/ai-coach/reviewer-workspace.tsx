...
export function ReviewerWorkspace({ initial }: { initial: Queue }) {
  ...
  return <div className="mx-auto max-w-4xl space-y-6 px-4 py-8 text-zinc-100"><header><p className="text-blue-400">Steadfast AI Coach · Synthetic reviewer workspace</p><h1 className="mt-2 text-3xl font-semibold">Proposal review queue</h1><p className="mt-3 text-zinc-400">{queue.backlog} pending proposals · queue capacity {queue.capacity}. This tool does not establish real staffing or clinical approval.</p>{queue.oldestPendingAt && <p className="mt-2 text-sm text-zinc-400">Oldest pending: {new Date(queue.oldestPendingAt).toLocaleString()}</p>}</header>
    <section className="space-y-3 rounded-2xl border border-white/10 p-5" aria-label="Assigned operational health"><h2 className="text-xl font-semibold">Assigned operational health</h2><p className="text-sm text-zinc-400">Last seven days · sampled {new Date(queue.operations.sampledAt).toLocaleString()}</p><dl className="grid gap-3 sm:grid-cols-2">{Object.entries({ "Failed runs": queue.operations.statuses.FAILED ?? 0, "Retried runs": queue.operations.retriedRuns, "Expired worker leases": queue.operations.expiredLeases, "Undelivered notification intents": queue.operations.pendingNotifications, "Closed-window proposals": queue.operations.closedPendingProposals }).map(([label, value]) => <div key={label}><dt className="text-zinc-400">{label}</dt><dd className="text-xl">{value}</dd></div>)}</dl><p className="text-sm text-zinc-400">Oldest open run: {queue.operations.oldestOpenRunAt ? new Date(queue.operations.oldestOpenRunAt).toLocaleString() : "None"}. Notification delivery is not configured; the outbox records intent only. Live alert thresholds and owners still require operational approval.</p><button className="min-h-12 rounded-xl border border-white/20 px-4" onClick={() => window.location.reload()}>Refresh operational state</button></section>
    <SafetyReview />
